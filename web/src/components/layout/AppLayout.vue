...
<template>
  <div class="min-h-screen flex flex-col bg-neutral-50">
    ...
    <header class="sticky top-0 z-30 bg-surface border-b border-neutral-200">
      ...
      <!-- Active supplier banner — CUSTOM(fork): zobrazit „Pracuješ jako" i při jediné firmě
           (přepínač uvnitř se stejně ukáže jen u >1 firmy díky vlastnímu v-if). -->
      <div v-if="supplierStore.currentSupplier" class="bg-primary-50 border-t border-primary-100">
        <div class="px-4 py-2 text-sm text-primary-700 flex items-center gap-2">
          <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
