...
final class PdfArchiveService
{
    ...
    public function deleteArchiveEntry(int $archiveId, int $invoiceId): bool
    {
        ...
        // CUSTOM(fork): cestu řeší upstream archiveFilePath() (měsíční podsložka + fallback na
        // ploché _archive), ať delete najde soubor i v novém shardovaném úložišti (4.39+).
        $supplierId = $this->resolveSupplierId($invoiceId);
        $path = $this->archiveFilePath($supplierId, (string) $filename);
        if (is_file($path)) {
            @unlink($path);
