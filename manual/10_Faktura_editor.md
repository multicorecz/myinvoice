...
[tvůj dodavatel] → Číslování faktur** — viz [§ 33.5.3](33_Multi_supplier.md#3353-cislovani-faktur).
...
- **Předvyplnění per dodavatel:** výchozí režim nové faktury nastavíš v
  **Nastavení → Můj dodavatel → Ceny s DPH** (viz [§ 33.3](33_Multi_supplier.md#333-co-je-per-dodavatel-izolovane)).
...
| Jednotka | Z číselníku (default `h` / hodina). Číselník spravuješ v **Systém → Číselníky → Jednotky** — viz [§ 34.1.4](34_Nastaveni.md#3414-jednotky). |
